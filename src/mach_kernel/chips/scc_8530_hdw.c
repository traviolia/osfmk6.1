...
#else
#define	SCC_READ_REG		scc_read_reg
#define	SCC_WRITE_REG		scc_write_reg
#endif
	...
	struct bus_device *ui;
{
	...
	{ 
		...
		for (i = 0; i < NSCC_LINE; i++) {
			...
			tp->t_addr = (char*)(0x80000000 + (sccunit<<1) + (i&1));
			/* do min buffering */
			tp->t_state |= TS_MIN;
		}
	}
...
#if	NBM > 0
	if (SCREEN_ISA_CONSOLE() && scc_isa_console) {
		printf("\n sl0: ");
...
/*	splx(s);*/
	...
	if ((mi_line == SCREEN_LINE_KEYBOARD) && (from_line == SCC_CHANNEL_A) &&
	    !raw && SCREEN_ISA_CONSOLE() && scc_isa_console)
