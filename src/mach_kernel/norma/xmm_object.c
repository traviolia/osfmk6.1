	...
	ipc_port_t	memory_object)
{
	...
	assert(IP_NORMA_IS_PROXY(xmm_object) ||
	       ip_kotype(xmm_object) == IKOT_XMM_OBJECT);
...
xmm_object_wait(
	ipc_port_t	memory_object)
{
	while (! IP_NORMA_IS_PROXY(memory_object) &&
		...
		assert_wait((int) &memory_object->ip_norma_xmm_object, FALSE);
...
/*
 * xmm_object_copy is an unused parameter: it is always a copy
 * of xmm_object and serves to preserve the correct number of send
 * rights on remote invocations.
 */
kern_return_t
norma_xmm_object_nuke(
	ipc_port_t	xmm_object,
	ipc_port_t	xmm_object_copy)
{
	xmm_obj_t mobj;
	...
	ipc_port_t	memory_object)
{
	...
	if (--memory_object->ip_norma_xmm_object_refs == 0) {
		...
		if (IP_NORMA_IS_PROXY(xmm_object)) {
			r_norma_xmm_object_nuke(xmm_object, xmm_object);
		} else {
			norma_xmm_object_nuke(xmm_object, xmm_object);
		}
	} else
		ip_unlock(memory_object);
}
	...
	ipc_port_t	memory_object)
{
	...
	if (wanted)
		thread_wakeup((int) &memory_object->ip_norma_xmm_object);
}
	...
	ipc_port_t	memory_object)
{
	 ...
	 * the send right, and the no-sender XXX s notification handler
	...
	xmm_object_wait(memory_object);
	if (IP_NORMA_IS_PROXY(memory_object)) {
		old_xmm_object = xmm_object;
		xmm_object_destroy(xmm_object, mobj);
		/* the following call inherits the lock */
		xmm_object = xmm_object_by_memory_object_remote(memory_object);
		/* lock on memory_object has been consumed */
		xmm_object_destroy(old_xmm_object, mobj);
		return (xmm_object);
	}
 ...
 */
kern_return_t
norma_xmm_object_by_memory_object(
	host_t		host,
	ipc_port_t	memory_object,
	ipc_port_t	*xmm_object)
{
	...
	ipc_port_t	memory_object)
{
	...
	node = memory_object->ip_norma_dest_node;
	ip_unlock(memory_object);
	r_norma_xmm_object_terminate(remote_host_priv(node,
						      MACH_MSG_TYPE_PORT_SEND),
				     memory_object->ip_norma_uid,
				     memory_object);
}
 ...
 */
int xmm_object_release_fast = 0;
int xmm_object_release_slow = 0;
int xmm_object_fast = 0;
void
xmm_object_release(
	ipc_port_t	memory_object)
{
	 ...
	 */
	if (xmm_object_fast &&
	    ! IP_NORMA_IS_PROXY(memory_object) &&
	    memory_object->ip_norma_xmm_object_refs == 1 &&
	    ! IP_NORMA_IS_PROXY((xmm_object =
				 memory_object->ip_norma_xmm_object))) {
		xmm_obj_t mobj = (xmm_obj_t) xmm_object->ip_kobject;

		xmm_object_release_fast++;
		assert(xmm_object != memory_object);
		memory_object->ip_norma_xmm_object_refs = 0;
		memory_object->ip_norma_xmm_object = memory_object;
		ipc_kobject_set(xmm_object, IKO_NULL, IKOT_NONE);
	...
	ipc_port_t	memory_object)
{
	...
	if (IP_NORMA_IS_PROXY(memory_object)) {
		node = memory_object->ip_norma_dest_node;
		ip_unlock(memory_object);
		return r_norma_xmm_object_reference(
	...
	ipc_port_t	memory_object)
{
	 ...
	 */
	xmm_object_terminate(memory_object);
