	...
	exception_flavor_array_t	exc_flavors)
{
	...
	ipc_port_t port_registered[TASK_PORT_REGISTER_MAX+1];
	ipc_port_t port_exception[EXC_TYPES_COUNT];
	ipc_port_t port_bootstrap;
	unsigned i;
	unsigned j;
	 ...
	 */
	port_bootstrap = task->itk_bootstrap;
	 ...
	 */
	for (i = 0; i < count; i++) {
		port_registered[i] = task->itk_registered[i];
	 ...
	 */
	for (i = 0; i < exc_count; i++)
		port_exception[i] = MACH_PORT_NULL;
	...
	for (i = 0; i < exc_count; i++) {
		...
		while (mask) {
			if (mask & 1) {
				port_exception[j] = task->exc_actions[j].port;
	...
	/*
	 * Release send rights of replaced ports without lock held.
	 */
	if (IP_VALID(port_bootstrap))
		ipc_port_release_send(port_bootstrap);
	for (i = 0; i < count; i++)
		if (IP_VALID(port_registered[i]))
			ipc_port_release_send(port_registered[i]);

	for (i = 0; i < exc_count; i++) {
		if (IP_VALID(port_exception[i]))
			ipc_port_release_send(port_exception[i]);
	...
	task_t		*child_task)
{
	...
	kr = norma_get_special_port((host_t) 1, child_node,
				    NORMA_HOST_PRIV_PORT, &remote_host);
	if (kr) {
		panic("norma_task_create: norma_get_special_port: %d\n", kr);
		return kr;
	}
