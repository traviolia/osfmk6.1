...
#define	m_msvm_data_write_completed	k_invalid_data_write_completed
...
#define	k_ksvm_data_write_completed k_invalid_data_write_completed
	...
	vm_prot_t	desired_access)
{
	...
	xmm_obj_lock(mobj);
#if	MACH_PAGEMAP
	if (! vm_external_null(&MOBJ->pagemap) &&
	...
	vm_size_t	length)
{
	...
	svm_data_write_completed(mobj, offset, REQUEST_NULL);
	return KERN_SUCCESS;
}
	...
	xmm_obj_t	mobj)
{
	...
	printf("num_pages=%d, ", MOBJ->num_pages);
	...
	iprintf("wc_pending=%d, %smodwc",
	       MOBJ->wc_pending,
	       BOOL(MOBJ->modwc));

#if	MACH_PAGEMAP
	printf(", pagemap=");
	(void) vm_external_print(&MOBJ->pagemap);
#endif	/* MACH_PAGEMAP */
	printf("\n");
	...
	xmm_obj_t	kobj)
{
	...
	iprintf("k_count=0x%x wc_pending=%d %sneeds_terminate\n", 
		KOBJ->k_count,
		KOBJ->wc_pending,
	...
	int		flags)
{
	...
	iprintf("%stemporary, request_count=%d, kobj_count=%d\n",
		...
		MOBJ->request_count,
		MOBJ->kobj_count);
	iprintf("k_count=%d wc_pending=%d lock_batching=%d\n",
		MOBJ->k_count,
		MOBJ->wc_pending,
		MOBJ->lock_batching);
	indent -= 4;
