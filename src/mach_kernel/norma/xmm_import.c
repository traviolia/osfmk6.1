	...
	xmm_obj_t	*new_mobj)
{
	...
	queue_init(&MOBJ->current_ipc);
	queue_init(&MOBJ->pending_ipc);
...
#endif	/* lint */
	return proxy_freeze(MOBJ->xmm_pager, existence_map, existence_size);
}
	...
	int		flags)
{
	...
	iprintf("current_ipc {next=0x%x, prev=0x%x}\n",
		MOBJ->current_ipc.next,
		MOBJ->current_ipc.prev);
	iprintf("pending_ipc {next=0x%x, prev=0x%x}\n",
		MOBJ->pending_ipc.next,
		MOBJ->pending_ipc.prev);
	indent -= 4;
	printf("----------Continued on Node %d UID 0x%x----------\n",
	...
	ipc_port_t	xmm_kernel)
{
	...
	ipc = xmm_ipc_enter(xmm_kernel, kobj, &KOBJ->current_ipc,
			    &KOBJ->pending_ipc, TRUE, 0, 0);
...
#define	UGLY_HACK 1
	...
	xmm_obj_lock(kobj);
	xmm_ipc_exit(kobj, ipc, &KOBJ->current_ipc,
		     &KOBJ->pending_ipc); /* consumes lock */
	...
	vm_size_t	length)
{
	...
	ipc = xmm_ipc_enter(xmm_kernel, kobj, &KOBJ->current_ipc,
			    &KOBJ->pending_ipc, FALSE, offset, length);
	xmm_obj_unlock(kobj);
...
/*
 * Because an m_o_destroy is asynchronous, we may have data_returns
 * pending on write_completed messages, but this is OK.  Not true
 * for terminate.
 */
kern_return_t
_proxy_destroy(
	...
	vm_size_t	size)
{
	...
	kobj = convert_xmm_kernel_to_kobj(xmm_kernel);
	if (kobj == XMM_OBJ_NULL) {
		printf("proxy_data_write_completed:ERROR: kernel 0x%x\n", xmm_kernel);
		return KERN_FAILURE;
	}
	xmm_ipc_critical(xmm_kernel);
	...
	vm_size_t	size)
{
	...
	ipc = xmm_ipc_enter(xmm_kernel, kobj, &KOBJ->current_ipc,
			    &KOBJ->pending_ipc, FALSE, offset, size);
	xmm_obj_unlock(kobj);
