...
struct mobj {
	...
	int		p_count;		/* XXX outstanding WCs */
	...
	if (internal) {
		...
		return K_SET_READY(mobj, OBJECT_READY_TRUE, MAY_CACHE_FALSE,
				   ...
				   ipc_port_make_send(MOBJ->
						      memory_object_name),
				   XMM_REPLY_NULL, /*TEMPORARY_*/TRUE,
				   (char *) 0, 0);
	} else {
...
#ifdef	MACH_ASSERT
	/*
	 * Verify that pages givne to the default pager are non-pageable.
	 */
	...
	int		flags)
{
	...
	indent += 2;
	iprintf("mem_object=0x%x, mem_control=0x%x, mem_object_name=0x%x\n",
		MOBJ->memory_object,
		MOBJ->memory_control,
		MOBJ->memory_object_name);
	iprintf("wc_count=%d, %sinitialized, %smodwc, %sinvalidate\n",
		MOBJ->p_count,
	...
	vm_prot_t	lock_value)
{
	...
	kr = K_DATA_SUPPLY(kobj, offset, (vm_map_copy_t)data,
			   length, lock_value, PRECIOUS_FALSE, XMM_REPLY_NULL);
	xmm_obj_lock(kobj);
	...
	mach_msg_type_name_t		reply_to_type)
{
	...
	case MEMORY_OBJECT_BEHAVIOR_INFO:
		if (count < MEMORY_OBJECT_BEHAVE_INFO_COUNT)
			...
			write_completions = attr->write_completions;
...
memory_object_representative(
	host_t			host,
        ...
        security_token_t	sec_token)
{
	panic("memory_object_representative is not implemented\n");
	return KERN_FAILURE;
}
#endif	/* MOR */
