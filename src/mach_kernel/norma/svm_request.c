...
#define AFALSE FALSE
		...
		/* boolean_t */	wc_pending:1,
		/* boolean_t */	wc_waiting:1,
...
#define	xmm_req_lock_init(req)	spinlock_init(&(req)->slock)
#define	xmm_req_lock(req)	spinlock_lock(&(req)->slock)
#define	xmm_req_unlock(req)	spinlock_unlock(&(req)->slock)

#define	INTERNAL_KOBJ	(xmm_obj_t)0xdeadbeef
	...
	vm_prot_t	lock)
{
	...
	if (lock == VM_PROT_NO_CHANGE)
		...
		K_GET_PROT(k_bits, protval);
		flush = (lock == VM_PROT_ALL) ||
			(protval && ((protval & ~lock) == VM_PROT_NONE));
		if (flush) {
 ...
 *
 * If kobj == XMM_OBJ_NULL, then data is from an unknown kernel, except
 * if there is a pending data_write_completed.
 */
void
 ...
 *		assert(xmm_req_lock_held(req)); */
	...
	if (! M_GET_PRECIOUS(m_bits) && ! dirty) {
		xmm_obj_unlock(mobj);
		if (MOBJ->modwc && kobj == XMM_OBJ_NULL)
			svm_data_write_completed(mobj, offset, req);
		else {
			if (req)
				xmm_req_unlock(req);
			if (MOBJ->modwc)
				svm_send_write_completed(kobj, mobj, offset);
		}
	...
	if (MOBJ->modwc && kobj != XMM_OBJ_NULL)
		svm_add_completion_pending(mobj, kobj, offset, req, FALSE);
	else {
		xmm_obj_unlock(mobj);
		if (req) {
			assert(!MOBJ->modwc || req->wc_pending);
			xmm_req_unlock(req);
		} else
...
int c_krequest_dup = 0;

/*
 * Add the necessary completion pending information to a request.
 *
 * MP note : Upon call, mobj and non-NULL req are locked;
 *	Upon return, all locks are consumed.
 */
void
svm_add_completion_pending(
	xmm_obj_t	mobj,
	xmm_obj_t	kobj,
	vm_offset_t	offset,
	request_t	req,
...
/*	assert(xmm_req_lock_held(req)); */
	...
	/*
	 * On a request, there may be more than one pending write_completion:
	 * Suppose a kernel returns a page that another kernel has claimed
	 * for. The pager may satisfy the request before it has sent the
	 * m_o_data_write_completed for the initial page (it depends whether
	 * the pager waits for the return of m_o_data_write_completed call to
	 * manage the next request on the same object/page or if it considers
	 * that it can handle the next request before sending the write
	 * completion). Suppose now that the second kernel returns the page
	 * before the m_o_data_write_completed of the first page has been
	 * received, then there will be two pending write completions. The
	 * potential maximum number of pending write completions is the number
	 * of pager threads. On a MP system, if the processor that currently
	 * handles the m_o_data_write_completed is preempted, the same condition
	 * may occur
	 */
	while (req->wc_pending) {
		req->wc_waiting = TRUE;
		assert_wait((int)&req->wc_pending_kobj, FALSE);
		xmm_req_unlock(req);
	...
	if (! fictitious) {
		assert(KOBJ->wc_pending > 0);
		req->wc_pending_kobj = kobj;
	} else {
		req->wc_pending_kobj = INTERNAL_KOBJ;
	}
	xmm_req_unlock(req);
}
...
/*	assert(xmm_req_lock_held(req)); */
	...
	if (IS_REAL_DATA(req->data) && !(req->lock & VM_PROT_WRITE) &&
		...
		if (req->wc_pending) {
			assert(req->wc_pending_kobj != INTERNAL_KOBJ);
			svm_data_write_completed(mobj, req->offset, req);
		} else
			xmm_req_unlock(req);
	...
	if (req->wc_pending && IS_REAL_DATA(req->data) && !kreq->needs_data) {
		/*
		 * This code covers the following race condition:
		 * 1) The kernel sends a m_o_data_unlock to XMM.
		 * 2) XMM sends a m_o_lock_request, since the XMM
		 *	object has a copy.
		 * 3) The kernel replies by a m_o_data_return and a
		 *	m_o_lock_completed.
		 * 4) XMM restarts processing the initial m_o_data_unlock
		 *	in this procedure.
		 *
		 * Unfortunately, the copy object has been removed
		 * during that time. So, we have to clean up request state.
		 */
		assert(req->wc_pending_kobj == kobj);
		assert(req->needs_data);
		data = req->data;
		req->data = DATA_NONE;
		svm_data_write_completed(mobj, req->offset, req);
	...
	xmm_obj_lock(mobj);
	M_GET_LOCK(m_bits, current_lock);
	xmm_obj_unlock(mobj);
	...
	if (req->data == DATA_UNAVAILABLE) {
		if (req->wc_pending) {
			/*
			 * A race condition has occured between the current
			 * mobj and its copy. The page has been copied from
			 * the copy object, and the mobj has received the
			 * data to supply after the page fault. So, retry the
			 * page fault at vm_fault() layer, since the pager
			 * must have a copy of the page.
			 */
			assert(req->wc_pending_kobj == INTERNAL_KOBJ);
			xmm_req_unlock(req);
			kr = K_DATA_ERROR(kobj, req->offset, PAGE_SIZE,
	...
	/*
	 * Data_supply requires a page-list flavor copy object,
	 * but data might be an object flavor copy object if
	 * it arrived from a local kernel as result of a lock_request.
	 */
	if (req->data->type != VM_MAP_COPY_PAGE_LIST) {
		xmm_req_unlock(req);
		kr = vm_map_convert_to_page_list(&req->data);
		assert(kr == KERN_SUCCESS);
		xmm_req_lock(req);
	}
	...
	/*
	 * Return the m_o_data_write_completed only if the supplied kernel is
	 * different from the one which returned data. If they are the same,
	 * the m_o_data_write_completed should be returned by the pager.
	 */
	xmm_req_lock(req);
	if (req->wc_pending &&
	    req->wc_pending_kobj != kobj &&
	...
	if (svm_copy_call_push_page_to_kernel && kr == KERN_INVALID_ARGUMENT) {
		...
		xmm_obj_lock(mobj);
		MOBJ->k_count++;
		if (MOBJ->modwc) /* XXX Rewrite lock and kcount management */
			svm_add_completion_pending(mobj, kobj,
						   req->offset, req, TRUE);
		/* consumes all locks */
...
/*			assert(xmm_req_lock_held(r)); */
	...
	req->wc_pending = FALSE;
	req->wc_waiting = FALSE;
	req->needs_data = FALSE;
	req->active = AFALSE;
...
/*	assert(xmm_req_lock_held(req)); */
	...
	if (req->prequest_list != PREQUEST_NULL ||
	    req->krequest_list != KREQUEST_NULL || req->wc_pending) {
		/*
...
_All_Completed:
	 ...
	 */
	req->active = AFALSE;
	if (req->prequest_list || req->krequest_list || req->wc_pending) {
		xmm_req_unlock(req);
	} else {
	...
	if (req->state != Pager_Pending) {
		...
		if (IS_REAL_DATA(data)) {
			if (precious) {
				 ...
				 * Needs more thought
				 *
				 * data_write_completed's are a problem -
				 * the pager will automatically generate
				 * a d_w_c but no one is interested in
				 * it.  Will have to detect bogus cases
...
add_trace(kobj, offset, dirty, kernel_copy, req); /* XXX */
	...
	if (! req->needs_data) {
		...
		if (MOBJ->modwc) {
			/*
			 * Establish a record to get back to a kernel.
			 */
			svm_add_completion_pending(mobj, kobj,
						   offset, req, FALSE);
		} else {
	...
	request_t	req)
{
	...
	if (! req) {
		...
		xmm_list_unlock(mobj);
		/*
		 * If no request is present then bail out -
		 * we must not be supporting modwc.  Note that
		 * this case is really the exception rather
		 * than the rule (hence we don't check for
		 * MOBJ->modwc in all cases).
		 */
		if (req == REQUEST_NULL) {
                	assert(!MOBJ->modwc);
			return;
	...
	/*
	 * The kobj will wait for the last completed message to
	 * arrive, so a req better be present.
	 */
	assert(req);
/*	assert(xmm_req_lock_held(req)); */
	assert(req->wc_pending);

	kobj = req->wc_pending_kobj;
	req->wc_pending = FALSE;
	if (req->wc_waiting) {
		req->wc_waiting = FALSE;
		thread_wakeup((int)&req->wc_pending_kobj);
		xmm_req_unlock(req);
	} else if (! req->krequest_list && ! req->prequest_list)
		svm_release_request(req); /* consumes lock */
	else
		xmm_req_unlock(req);

	if (kobj == INTERNAL_KOBJ) {
		/*
		 * Release the reference taken in svm_add_completion_pending.
		 */
		xmm_obj_lock(mobj);
		if (--MOBJ->k_count == 0 &&
		    MOBJ->state == MOBJ_STATE_SHOULD_TERMINATE) {
...
int reconsider_count = 0;
xmm_obj_t last_reconsider;

void
svm_send_write_completed(
	xmm_obj_t	kobj,
	xmm_obj_t	mobj,
	vm_offset_t	offset)
{
	...
	assert(MOBJ->wc_pending > 0);
	MOBJ->wc_pending--;
	xmm_obj_unlock(mobj);

	/*
	 * Watch for the special case where there is no valid kobj
	 */
	assert(kobj != INTERNAL_KOBJ);
#if	!MACH_ASSERT
	if (kobj == INTERNAL_KOBJ)
		return;
#endif	/* !MACH_ASSERT */
	...
	kr = K_DATA_WRITE_COMPLETED(kobj, offset, PAGE_SIZE);
	assert(kr == KERN_SUCCESS);

	/*
	 * See if there is any cleanup processing pending
	 */
	xmm_obj_lock(mobj);
	xmm_obj_lock(kobj);
	if (KOBJ->needs_terminate && KOBJ->wc_pending == 0) {
		xmm_obj_unlock(mobj);
		xmm_svm_terminate(kobj, TRUE); /* consumes lock */

	} else if (MOBJ->destroy_needed && MOBJ->wc_pending == 0) {
		/*
		 * XXX does this case ever happen?
		 */
		xmm_obj_unlock(kobj);
		reconsider_count++;
		last_reconsider = mobj;
...
svm_request_init(void)
{
	...
	(void)zcollectable(svm_request_zone);
	(void)zcollectable(svm_krequest_zone);
	(void)zcollectable(svm_prequest_zone);
}
	...
	request_t	req)
{
	...
	iprintf("next_chain{next=0x%x, prev=0x%x}\n",
		req->next_chain.next,
		req->next_chain.prev);
	iprintf("%swc_pending, %swc_waiting, wc_pending_kobj=0x%x\n",
		BOOL(req->wc_pending),
		BOOL(req->wc_waiting),
		req->wc_pending_kobj);
#if MACH_ASSERT
