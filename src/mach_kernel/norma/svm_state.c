	...
	unsigned int	page)
{
	...
	for (k = MOBJ->kobj_list; k; k = K->next) {
		...
		if (k_bits != (char *)0) {
			K_GET_PROT(k_bits, k_prot);
			if (k_prot && ! K_GET_PRECIOUS(k_bits)) {
				xmm_obj_unlock(k);
				return k;
			}
		}
