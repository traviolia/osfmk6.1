...
thread_t	active_threads[NCPUS];	/* current active threads	*/
...
spinlock_t		stack_lock_data;		/* splsched only */
#define stack_lock()	spinlock_lock(&stack_lock_data)
#define stack_unlock()	spinlock_unlock(&stack_lock_data)
	 ...
	 void)
{
	...
	thr_sh_template.swap_func = (void (*)(void))thread_bootstrap_return;
...
/*	thr_sh_template.sched_pri (later - compute_priority) */
	...
	spinlock_init(&stack_lock_data);
#endif  /* MACHINE_STACK */

#if	MACH_DEBUG
	spinlock_init(&stack_usage_lock);
#endif	/* MACH_DEBUG */
...
thread_terminate_self(void)
{
	...
	thread_act_t	thr_act = thread->top_act;	/* racy */
	...
	s = splsched();
	spinlock_lock(&reaper_lock);
	enqueue_tail(&reaper_queue, (queue_entry_t) thread);
	spinlock_unlock(&reaper_lock);
						...
						thread_t *child_thread)
{
	...
	*new_thread = thr_sh_template;
	...
	act_reference(thr_act);	/* Thread holds a reference to the thr_act */
	...
	/* Check against pset limits */
	max = pset_max_priority(pset, new_thread->policy);
	if (max > new_thread->max_priority)
		new_thread->max_priority = max;
 ...
 */
thread_t
kernel_thread(task_t task, void (*start)(void), char *arg)
{
	kern_return_t	kr;
	thread_t	thread;
...
#if	MACH_HOST
		if (need_wakeup)
			thread_wakeup((int)&thread->assign_active);
#endif	/* MACH_HOST */
	...
	unsigned int		*thread_info_count)	/*IN/OUT*/
{
	...
	if (flavor == THREAD_BASIC_INFO) {
	    ...
	    basic_info->suspend_count = thread->top_act->user_stop_count;
...
reaper_thread(void)
{
	...
	for (;;) {
		...
		while ((thread = (thread_t) dequeue_head(&reaper_queue))
			...
			act_unlock_thread(thr_act);
			act_detach(thr_act, FALSE);
			/* Remove the reference held by the thread: */
			act_deallocate(thr_act);
			assert(thread->top_act == THR_ACT_NULL);
	...
	thread_t	thread)
{
	...
	while (thread->may_assign == FALSE) {
		...
		thread_sleep_spinlock((int) &thread->assign_active,
					&thread->lock, TRUE);
		thread_lock(thread);
	}
	...
	boolean_t			release_freeze)
{
	...
	/*
	 *	Have to release thread-related locks before acquiring pset
	 *	locks.
	 */
	act_unlock_thread(thr_act);
	...
	register vm_offset_t stack)
{
	if (stack_check_usage) {
	    ...
	    spinlock_lock(&stack_usage_lock);
	    if (used > stack_max_usage)
		stack_max_usage = used;
	    spinlock_unlock(&stack_usage_lock);
	...
	vm_offset_t	*maxstackp)
{
	...
	for (i = 0; i < actual; i++) {
		...
		stack = thread->top_act->kernel_stack;
