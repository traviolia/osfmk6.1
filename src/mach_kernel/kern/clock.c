...
spinlock_t		ClockLock;	/* clock system synchronization */
...
#define LOCK_CLOCK(s)			\
	s = splclock();			\
	spinlock_lock(&ClockLock);

#define UNLOCK_CLOCK(s)			\
	spinlock_unlock(&ClockLock);	\
	splx(s);
...
alarm_thread(void)
{
	...
	for (;;) {
	    ...
	    assert_wait((int)&alrmdone, FALSE);
	    UNLOCK_CLOCK(s);
	    thread_block((void (*)(void)) 0);
	}
    }
