	...
	mach_msg_type_number_t	*countp)
{
	...
	addr = ALLOC((vm_size_t) (count * sizeof(mach_port_t)), rt);
	if (addr == 0)
	...
	mach_msg_type_number_t	*count)
{
	...
	case HOST_SCHED_INFO:
		...
		*count = HOST_SCHED_INFO_COUNT;
		...
		{ register kernel_resource_sizes_t	kernel_resource_sizes;
                  /*
	...
	mach_msg_type_number_t	*count)
{
	...
	case HOST_VM_INFO: {
		...
		stat = (vm_statistics_t) info;
                ...
                stat->zero_fill_count = vm_stat.zero_fill_count;
                stat->reactivations = vm_stat.reactivations;
                stat->pageins = vm_stat.pageins;
                stat->pageouts = vm_stat.pageouts;
                stat->faults = vm_stat.faults;
                stat->cow_faults = vm_stat.cow_faults;
                stat->lookups = vm_stat.lookups;
                stat->hits = vm_stat.hits;
	...
	mach_msg_type_number_t		*count)
{
	...
	addr = ALLOC((vm_size_t) sizeof(mach_port_t), rt);
	if (addr == 0)
		return KERN_RESOURCE_SHORTAGE;
