	...
	register processor_set_t	pset)
{
	...
	/* setup run-queues */
	spinlock_init(&pset->runq.lock);
	pset->runq.count = 0;
	for (i = 0; i < NRQBM-1; i++) {
	    pset->runq.bitmap[i] = 0;
	}
	pset->runq.bitmap[i] = 0x80000000;	/* set bit 127 */
	pset->runq.low = IDLEPRI;		/* set priority 127 */
	for (i = 0; i < NRQS; i++) {
	...
	spinlock_init(&pset->quantum_adj_lock);
...
#if	NCPUS > 1
	/* setup run-queues */
	spinlock_init(&pr->runq.lock);
	pr->runq.count = 0;
	for (i = 0; i < NRQBM-1; i++) {
	    pr->runq.bitmap[i] = 0;
	}
	pr->runq.bitmap[i] = 0x80000000;	/* set bit 127 */
	pr->runq.low = IDLEPRI;			/* set priority 127 */
	for (i = 0; i < NRQS; i++) {
	...
	if (actual == 0) {
		...
		*count = actual;
		...
		switch (type) {
			...
			break;
#else
			panic("XXXXX processor_set_threads");
#endif	/* XXXXX */
		    }
		}
	}
