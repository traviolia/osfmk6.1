...
unsigned int	active_lock_sets  = 0;
...
lock_set_destroy (task_t task, lock_set_t lock_set)
{
	...
	ls_port = lock_set->port;
	ipc_kobject_set(ls_port, IKO_NULL, IKOT_NONE);
	lock_set->port = IP_NULL;
	lock_set->active = FALSE;

	/* Don't need the lock set's lock anymore, since it's deactivated */
	lock_set_unlock(lock_set);
	...
	ipc_port_dealloc_kernel(ls_port);
	kfree((vm_offset_t) lock_set, size);
...
kern_return_t
lock_assume_ownership (lock_set_t lock_set, int lock_id)
{
	ulock_t   ulock;
	...
	/*
	 *  Ownership can only be assumed if the lock is already held.
	 */

	if (ulock->state == ULOCK_FREE) {
		ulock_unlock(ulock);
		return KERN_FAILURE;
	}
	 ...
	 *
	 *  from the thread currently holding the ulock
	 *  to the calling thread.
	 */

	if (ulock->owner != THR_ACT_NULL)
		ulock_ownership_clear(ulock);
	ulock_ownership_set(ulock, current_thread());
