 ...
 */
	...
	/* Self-preservation */
	spinlock_t	lock;		/* scheduling lock (thread_lock()) */
	mutex_t		rpc_lock;	/* RPC lock (rpc_lock()) */
	spinlock_t	wake_lock;	/* covers wake_active (wake_lock())*/
...
#define	TH_SWAP_STATE	(TH_SWAPPED | TH_SW_COMING_IN)
	...
	/* Swapping/resume information */
	void		(*swap_func)(	/* start here next time runnable */
				void);
...
#define	ith_tss_thread		saved.thread_switch_save.thread
#define	ith_tss_option		saved.thread_switch_save.option
...
#define SAFE_MACH_MSG_RECEIVE		-3
...
extern thread_state_flavor_t thread_flavor_max;
...
#define thread_start(thread, start) (thread)->swap_func = (start)
...
#define	thread_lock_init(th)	spinlock_init(&(th)->lock)
#define thread_lock(th)		spinlock_lock(&(th)->lock)
#define thread_unlock(th)	spinlock_unlock(&(th)->lock)
...
#define rpc_lock_init(th)	mutex_init(&(th)->rpc_lock)
...
#define wake_lock_init(th)	spinlock_init(&(th)->wake_lock)
#define wake_lock(th)		spinlock_lock(&(th)->wake_lock)
#define wake_unlock(th)		spinlock_unlock(&(th)->wake_lock)
...
#define current_act_fast(cpu)	(active_threads[(cpu)]->top_act)
...
#define	current_task()		(current_act_fast(cpu_number())->task)
#define	current_map()		(current_act_fast(cpu_number())->map)
