	...
	task_t task)
{
	...
	while (task->may_assign == FALSE) {
		...
		thread_sleep_mutex((int) &task->assign_active, &task->lock,
				TRUE);
		task_lock(task);
	}
	...
	task_t			*child_task)
{
	...
	if ((retval = kmem_suballoc(kernel_map, &map_base,
		map_size, TRUE, FALSE, &new_task->map)) != KERN_SUCCESS) {
		/*
	...
	(void)task_set_info(new_task,
		TASK_USER_DATA, (task_info_t)&user_data, TASK_USER_DATA_COUNT);
	*child_task = new_task;

#if	MACH_KDB
	/*
	 * The following call is a KLUDGE to clone the osf1_server's symbol
	 * table with offset addresses for this (first) INKserver.
	 */
	{   extern void db_clone_symtabXXX(char *, char *,vm_offset_t, char *);
	    db_clone_symtabXXX("unix", "inks", map_base, (char*)new_task->map);
	}
#endif	/* MACH_KDB */
	...
	task_t		*child_task)		/* OUT */
{
	...
	return(task_create_local(parent_task,
 ...
 *	Task is locked.
 */
void task_free( register task_t	task )
{
	register int c;
	register processor_set_t pset;
	...
	register task_t	task)
{
	 ...
	 *	However, we just call thread_force_terminate(), which
	 ...
	 *
        ...
        while (!queue_empty(list)) {
                ...
                task_unlock(task);
                thread_force_terminate(thr_act);
	...
	if (cur_thr_act->task == task) {
		...
		task_unlock(task);
		(void) thread_terminate(cur_thr_act);
	}
 ...
 *	task_dowait:
 *
 *	Wait until the task has really been suspended (all of its threads
...
{
	register queue_head_t	*list;
	register thread_act_t	thr_act, cur_thr_act, prev_thr_act;
	register kern_return_t	ret = KERN_SUCCESS;

	/*
	 *	Iterate through all the thread_act's.
	 *	While waiting for each thread_act, we gain a reference to it
	 *	to prevent it from going away on us.  This guarantees
	 *	that the "next" thread_act in the list will be a valid thread.
	 *
	...
	while (!queue_end(list, (queue_entry_t) thr_act)) {
		...
		if (thr_act != cur_thr_act) {
			act_reference(thr_act);
			task_unlock(task);
			if (prev_thr_act != THR_ACT_NULL)
				act_deallocate(prev_thr_act);
							/* may block */
			(void) act_lock_thread(thr_act);
			(void) thread_dowait(thr_act, TRUE);  /* may block */
			(void) act_unlock_thread(thr_act);
			prev_thr_act = thr_act;
			task_lock(task);
		}
		thr_act = (thread_act_t) queue_next(&thr_act->thr_acts);
	}
	task_unlock(task);
	if (prev_thr_act != THR_ACT_NULL)
		act_deallocate(prev_thr_act);		/* may block */
	return(ret);	
}
	...
	boolean_t			assign_threads)
{
	...
	while (task->may_assign == FALSE) {
		task->assign_active = TRUE;
		assert_wait((int)&task->assign_active, TRUE);
		task_unlock(task);
		thread_block((void (*)(void)) 0);
		task_lock(task);
	}
	...
	boolean_t	change_threads)
{
	...
	if (change_threads) {
		register thread_act_t	thr_act, nextp;
		register thread_t	thread;

		for (thr_act  = (thread_act_t)queue_first(&task->thr_acts);
		     thr_act != (thread_act_t)&task->thr_acts;
		     thr_act  = nextp) {
			nextp = (thread_act_t)queue_next(&thr_act->thr_acts);

			thread = act_lock_thread(thr_act);
			/* Do not muck about with immigrants' priorities */
			if (!thr_act->pool_port) {
			    assert(thread);
			    if (thread_priority_locked(thread,
				priority, FALSE) != KERN_SUCCESS)
				ret = KERN_FAILURE;
			}
			act_unlock_thread(thr_act);
		}
