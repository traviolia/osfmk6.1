	...
	thread_t		new)
{
...
#if 0
	log_thread_action (new, "switch_context");
#endif
...
#if MACH_RT
	assert (preemption_level[cpu_number()] == 1);
#endif
	...
	if (old_act->map != new_act->map) {
		PMAP_DEACTIVATE_USER(old_act, mycpu);
		PMAP_ACTIVATE_USER(new_act, mycpu);
	}
    }
...
kern_return_t
act_machine_set_state(thread_act_t thr_act, int flavor, thread_state_t tstate,
							unsigned count)
{
...
kern_return_t
act_machine_get_state(thread_act_t thr_act, int flavor, thread_state_t tstate,
							unsigned *count)
{
#if	MACH_ASSERT
	if (watchacts & WA_STATE)
	...
	vm_size_t	arg_size)
{
	...
	saved_state = USER_REGS(current_act());
...
void
act_machine_stack_attach(
	thread_act_t thr_act)
{
	MachineThrAct_t mact = &thr_act->mact;

	/*
	 * XXX - yetch:  get a kernel stack and associate it with
	 * the new activation.
	 */
	thr_act->kernel_stack = (int)stack_alloc();
	assert(thr_act->kernel_stack != 0);
	/*
	 *	Point top of kernel stack to user`s registers.
	 */
	STACK_IEL(thr_act->kernel_stack)->saved_state =
		&mact->pcb->iss;
}
...
act_machine_create(task_t task, thread_act_t thr_act)
{
	...
	/*
	 * Initialize the rest of the mact - non-zero parts only
	 *					FUTURES
	 */
	act_machine_stack_attach(thr_act);
...
act_machine_destroy(thread_act_t thr_act)
{
	...
	assert(thr_act->kernel_stack != 0);
	s = splsched();
	stack_free(thr_act->kernel_stack);
	splx(s);
...
act_machine_return(int code)
{
	...
	*/
	...
	/*
	 * XXXX the following is wrong:
	 *	(1) we aren't yet using kernel activations
	 *	(2) need to keep the thread ref for the reaper
	 */
	/* act_deallocate(thr_act);	* XXX thread reference */
	/* act_deallocate(thr_act);	* XXX kernel activation reference */
	thread_terminate_self();
	/*NOTREACHED*/
	panic("act_machine_return: TALKING ZOMBIE!");
}
...
dump_act(thread_act_t thr_act)
{
	...
	if (thr_act->thread && thr_act->kernel_stack) {
