	...
	struct bios_partition_info		*part)
{
	...
	ior->io_count = BOB_LABEL_SECTOR_SIZE;
	ior->io_error = 0;
	ior->io_op = IO_READ;
...
#endif
	label->d_secsize = BOB_LABEL_SECTOR_SIZE;
	...
	unsigned int	*status_count)
{
	...
	case V_RDABS:
		...
		if (scsi_rw_abs(dev, status, IO_READ, 
				scsi_abs_sec, tgt->block_size) != D_SUCCESS)
			return(D_INVALID_OPERATION);
	...
	unsigned int		status_count)
{
	...
	case V_WRABS:
		...
		if (scsi_rw_abs(dev, status, IO_WRITE, 
                    scsi_abs_sec, tgt->block_size) != D_SUCCESS)
			return(D_INVALID_OPERATION);
		break;
	default:
