...
void ns8390_multicast_create(
	...
	unsigned char	to[ETHER_ADDR_SIZE],
	unsigned char	crc[NS8390_MULTICAST_SIZE]);
...
#if	MBUS && NCPUS > 1
	spinlock_init(&ns8390_dev_lock[unit].lock);
...
ns8390getstat(
	dev_t		dev,
	int		flavor,
	int		*status,	/* pointer to OUT array */
	...
	unsigned int	count)
{
	...
	switch (flavor) {
		...
		net_multicast_create(new, (unsigned char *)status,
				     ...
				     ETHER_ADDR_SIZE);
		if ((sp->board_id & IFWD_NIC_690_BIT) == 0) {
			crc = NET_MULTICAST_MISC(new);
			ns8390_multicast_create(NET_MULTICAST_FROM(new),
						NET_MULTICAST_TO(new), crc);
		}

		oldpri = splimp();
		at386_io_lock(MP_DEV_WAIT);
		net_multicast_insert(&sp->multicast_queue, cur);
	...
	int	unit)
{
	...
	while(is->read_nxtpkt_ptr != ns8390get_CURR(unit))   {
		 ...
		 */
		if ((eh.ether_dhost[0] & 1) && /* multicast/broadcast */
...
packet_drop:
			...
			/*
			 * not only do we want to return, we need to drop the
		...
		*ehp = eh;
		if (len > wrap_size) {
			...
			if (board_id & IFWD_SLOT_16BIT) {
				s = splhi();
				en_16bit_access(hdwbase, board_id);
				bcopy16 ((char *) sram_getdata_ptr, (char *) (pkt + 1),
					 wrap_size);
				dis_16bit_access (hdwbase, board_id);
				splx(s);
			} else {
				bcopy ((char *) sram_getdata_ptr, (char *) (pkt + 1),
				       wrap_size);
			}
	...
	ns8390_softc_t		*is)
{
	...
	bcopy((caddr_t)eh, (caddr_t)ehp, sizeof(struct ether_header));
	bcopy((char *) &m->io_data[sizeof(struct ether_header)], 
	      (char *) (pkt + 1), m->io_count - sizeof(struct ether_header));
	pkt->type = ehp->ether_type;
	...
	net_packet(&is->ds_if, kmsg, pkt->length, ethernet_priority(kmsg));
}
	...
	int	unit)
{
	...
	outb (nic+RCR, (AB | AM |	/* receive configuration register */
			((is->ds_if.if_flags & IFF_PROMISC) ? PRO : 0)));
	...
	int	unit)
{
	...
	for (count = 0; count < 10000; count++)
		if (inb(nic+ISR) & RST)
			break;
	if (count == 10000)
		printf("%s%d: intoff(): would not reset.\n",
		       ns8390_softc[unit].card, unit);
	outb(nic+CR, temp_cr);
}
/*
...
ns8390_multicast_create(
	...
	unsigned char	crc[NS8390_MULTICAST_SIZE])
{
	unsigned char	addr[ETHER_ADDR_SIZE];
	unsigned char	c;
	unsigned long	crc32;
	unsigned char	*p;
	int		i;
	...
	while (addr[0]) {
	    /*
	     * Check next address is in the range.
	     */
	    for (i = 0; i < ETHER_ADDR_SIZE; i++)
		if (addr[i] != to[i])
		    break;
