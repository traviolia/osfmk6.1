	...
	int		npages)
{
	...
	ipl = splhi();
	spinlock_lock(&hil_lock);
	(hil+npages-1)->next = hil_head;
	hil_head = hil;
	spinlock_unlock(&hil_lock);
	splx(ipl);
}
	...
	hil_t		*hil)
{
	...
	while (!(h = hil_head)) { 
		...
		thread_sleep_spinlock((int)&hil_head, &hil_lock, FALSE);
		spinlock_lock (&hil_lock);
	}
	...
	if (io_op == D_WRITE) {
	  bcopy((char *)phystokv(phys_addr), (char *)phystokv(h->low_page),
		length);
	  h->length = 0;
	} else {
	  h->length = length;
	...
	hil_t		hil)
{
	...
	while(hil) {
		if (hil->length)
			bcopy((char *)phystokv(hil->low_page),
