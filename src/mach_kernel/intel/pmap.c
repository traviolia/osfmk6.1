...
extern boolean_t pmap_verify_free(
			vm_offset_t	phys);
...
extern void	phys_attribute_clear(
			vm_offset_t	phys,
			int		bits);

extern boolean_t phys_attribute_test(
			vm_offset_t	phys,
			int		bits);

extern void	set_dirbase(
			vm_offset_t	dirbase);
...
spinlock_t	pv_free_list_lock;

#define	PV_ALLOC(pv_e) { \
	spinlock_lock(&pv_free_list_lock); \
	if ((pv_e = pv_free_list) != 0) { \
	    pv_free_list = pv_e->next; \
	} \
	spinlock_unlock(&pv_free_list_lock); \
}
...
#if	MACH_RT
#define SPLVM(spl)			disable_preemption()
#define SPLX(spl)			enable_preemption()
#else	/* MACH_RT */
#define SPLVM(spl)
#define SPLX(spl)
#endif	/* MACH_RT */
...
vm_size_t morevm = 56 * 1024 * 1024;	/* VM space for kernel map */
	...
	vm_offset_t	load_start)
{
	...
	spinlock_init(&pv_free_list_lock);
	...
	vm_size_t	size)
{
	...
	spinlock_lock(&pmap_cache_lock);
	while ((p = pmap_cache_list) == PMAP_NULL) {

		vm_offset_t		dirbases;
		register int		i;

		spinlock_unlock(&pmap_cache_lock);
	...
	register pmap_t	p)
{
	 ...
	 */
	pdep = p->dirbase;
	while (pdep < &p->dirbase[pdenum(VM_MIN_KERNEL_ADDRESS)]) {
...
Retry:
	 ...
	 *	   1) If va was mapped, update TLBs, replace mapping and remove
	 *		old pvlist entry.  
	 *	   2) Add pvlist entry for new mapping
	 *	   3) If va was not mapped, enter new mapping.
	 *
	...
	if (old_pa != (vm_offset_t) 0) {
		...
		/*
		 *	Get the modify and reference bits and enter new
		 *	mapping.  Build a template to speed up entering -
...
#if	SHARING_FAULTS
	/*
	 *	Step 3) Mapping was not entered above under any
	 *	circumstances, so enter it now.
	 */
#else
	/*
	 *	Step 3) If old_pa is not zero, mapping was entered
	 *	in step 1), otherwise enter it here.
	 */
	if (old_pa == (vm_offset_t) 0)
#endif
	{
	...
	vm_offset_t	end)
{
	...
	while ((which_cpu = ffs((unsigned long)use_list)) != 0) {
	    ...
	    spinlock_lock(&update_list_p->lock);
	    ...
	    spinlock_unlock(&update_list_p->lock);
