...
extern boolean_t	device_pager_data_request_done(
				io_req_t		ior);
...
dev_pager_hash_init(void)
{
	...
	mutex_init(&dev_pager_hash_lock);
}
	...
	ipc_port_t	*pager)
{
	...
	if (device->dev_ops->d_mmap ==
		(vm_offset_t (*)(dev_t, vm_offset_t, int))block_io_mmap) {
...
#if	MOR
#define	memory_object_notify		device_pager_notify
#define	memory_object_rejected		device_pager_rejected
#endif	/* MOR */
...
memory_object_notify(
	ipc_port_t	representative,
        ipc_port_t	memory_control,
        host_t		host_name,
        vm_size_t	page_size)
{
	panic("(device_pager)notify: called");
	return (KERN_FAILURE);
}
...
{
	panic("(device_pager)rejected: called");
	return (KERN_FAILURE);
}
