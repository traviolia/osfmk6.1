 ...
 */
spinlock_t	net_thread_lock;
boolean_t	net_thread_awake = FALSE;

/*
 * Pool list.
 */
spinlock_t	net_pool_lock;
...
net_kmsg_collect(void)
{
	...
	spinlock_lock(&net_pool_lock);
	listp = net_pool_list;
	spinlock_unlock(&net_pool_lock);
	...
	ipc_kmsg_queue_t	send_list)
{
	...
	for (infp = (net_rcv_port_t) queue_first(&ifp->if_rcv_port_list);
	    ...
	    if (infp->filter == infp->filter_end ||
		(infp->fpstart ?
		 (*infp->fpstart)(
	...
	ipc_kmsg_t kmsg)
{
	...
	trailer = (mach_msg_format_0_trailer_t *)((int)&kmsg->ikm_header + 
		     (int)kmsg->ikm_header.msgh_size);                
	...
	boolean_t	nonblocking)
{
	...
	if ((kmsg = ipc_kmsg_dequeue(&p->net_queue_high)) != IKM_NULL) {
	    ...
	    return FALSE;
	spinlock_unlock(&p->net_queue_lock);
	(void) spllo();
	...
	(void) splimp();
	spinlock_lock(&p->net_queue_lock);
	return TRUE;
}
	...
	ipc_port_t	rcv_port)
{
	...
	if (!(ifp->if_flags & IFF_SG)) {
	    ...
	    spinlock_lock(&p->net_kmsg_total_lock);
	    p->net_queue_free_min--;
	    p->net_kmsg_max -= infp->rcv_qlimit + 1;
	    spinlock_unlock(&p->net_kmsg_total_lock);
	}
	mutex_unlock(&ifp->if_rcv_port_list_lock);
	...
	unsigned	len)
{
	...
	if (i == len) {
	    ...
	    bcopy((char *)from,
		  (char *)new + sizeof(net_multicast_t), len);
	} else {
	    new->hash = -1;
	    if (*p1 < *p2) {
		bcopy((char *)from,
	...
	if (new->size > NET_MULTICAST_LENGTH(cur->len))
		bzero((char *)NET_MULTICAST_MISC(cur),
		      new->size - NET_MULTICAST_LENGTH(cur->len));
}
	...
	net_pool_t *pool_result) /* New Pool Handle (OUT arg) */
{
	...
	*pool_result = NET_POOL_NULL;
	...
	spinlock_init(&p->net_queue_free_lock);
	spinlock_init(&p->net_kmsg_total_lock);
	spinlock_init(&p->net_queue_lock);
	...
	int *newmax)			/* # of buffers in pool (OUT arg) */
{
	...
	*newmax = p->net_kmsg_max;
	spinlock_unlock(&p->net_kmsg_total_lock);
	spinlock_lock(&p->net_queue_free_lock);
	p->net_queue_free_min += nbufs;
	spinlock_unlock(&p->net_queue_free_lock);
	return(KERN_SUCCESS);
}
	...
	net_pool_t	p)		/* Pool Handle */
{
	...
	if (net_kmsg_want_more(p) || (kmsg == IKM_NULL)) {
	    ...
	    s = splimp();
	    spinlock_lock(&net_thread_lock);
	    awake = net_thread_awake;
	    net_thread_awake = TRUE;
	    spinlock_unlock(&net_thread_lock);
	    (void) splx(s);
	...
	boolean_t		priority)
{
	...
	} else if (node_self_addr == (struct node_addr *) 0) {
		net_kmsg_put(kmsg);
		return;
	}
#endif	/* NORMA_ETHER */
...
net_io_init(void)
{
	...
	spinlock_init(&net_pool_lock);
	spinlock_init(&net_thread_lock);

	queue_init(&net_rcv_list);
	mutex_init(&net_rcv_list_lock);
