...
testdev_generate_packets(void)
{
	...
	for (i = 0; i < status->reply_count; i++) {
		...
		net_packet(&testdev_if, kmsg, status->reply_size, TRUE);
		if (! status->reply_synchronous) {
			thread_block((void (*)(void)) 0);
		}
	}
	...
	io_req_t	ior)
{
	...
	if (kr != KERN_SUCCESS) {
		printf("testdev_open: kmem_alloc returned %d.\n", kr);
		testdev_mem = NULL;
	}
	...
	unsigned int	fcount)
{
	...
	rc = net_set_filter(&testdev_if, rcv_port, pri, filter, fcount);
	return rc;
}
