...
{
	struct io_req iorq;
	spinlock_init(&iorq.io_req_lock);
	while (tp->t_outq.c_cc) {
		iorq.io_op = 0;
		queue_delayed_reply(&tp->t_delayed_write, &iorq,
				    (boolean_t (*)(struct io_req *)) 0);
		tp->t_state |= TS_ASLEEP;
		thread_sleep_spinlock((int) &iorq, &tp->t_lock, FALSE);
		spinlock_lock(&tp->t_lock);
		tp->t_state &= ~TS_ASLEEP;
	}
}
