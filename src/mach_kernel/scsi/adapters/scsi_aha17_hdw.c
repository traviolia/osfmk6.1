...

{
	...
	eaha_softc *eaha ;			/* TRM6-31..33 */
	{
		...
		_erccb->_eccb.scather =
			(caddr_t) kvtophys(eaha->host_inquiry_data) ;
	...
	target_info_t	*tgt)
{
	...
	if (tgt == 0)
		...
		tgt = scsi_slave_alloc(eaha - eaha_softc_data, id, (char *)eaha);
	...
	struct bus_ctlr		*ui)
{
	...
	/* bzero expects (char *) for generic 1st argument */
...
gotintr:
	...
	switch(g2intst>>4) {
		...
		default :
			log(	LOG_KERN,
				"eaha%d: Bogus status (x%x) in MBI\n",
				unit, mbi);
	...
	unsigned char	peer)
{
	...
	if (initiator == 0) {
		/* allocate */
		initiator = eaha_tgt_alloc(eaha, peer,
				/* Slave allocation when "target" = 0 */
				(target_info_t *) 0);
...
allok:
		...
		/* we'll get an interrupt soon */
                printf("eaha: bad PHASE sequencing\n");
		tgt->done = SCSI_RET_ABORTED;
		break;
