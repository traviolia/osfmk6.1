...
/*
 *	Function prototypes for unused commands
 */

/*	SCSI disk seek routine				*/
void	scsi_long_seek(
		target_info_t	*tgt,
		unsigned int	secno,
		io_req_t	ior);

/*	SCSI disk unreserve drive routine		*/
void	scsi_release(
		target_info_t	*tgt,
		unsigned char	id,
		unsigned char	mode,
...
/*	SCSI disk reserve blocks routine		*/
void	scsi_reserve(
		target_info_t	*tgt,
		unsigned int	len,
		unsigned char	id,
...
/*	SCSI disk soft reset routine			*/
void	scsi_rezero_unit(
		target_info_t	*tgt,
		io_req_t	ior);

/*	SCSI disk search routine			*/
void	scsi_search_data(
		target_info_t	*tgt,
		unsigned int	secno,
		unsigned char	how,
		unsigned char	flags,
...
/*	SCSI disk set limits routine			*/
void	scsi_set_limits(
		target_info_t	*tgt,
		unsigned int	secno,
...
/*	SCSI disk write/verify routine			*/
void	scsi_write_verify(
		target_info_t	*tgt,
		unsigned int	secno,
		io_req_t	ior);
